...
class HomeRemedyController extends Controller
{
    ...
    public function category(string $slug, Request $request) { $category=HomeRemedyCategory::where('slug',$slug)->where('is_active',true)->firstOrFail(); $q=trim((string)$request->query('q','')); $remedies=$category->remedies()->with(['category', 'ingredients'])->published()->when($q,fn($x)=>$x->where(fn($w)=>$w->where('title_en','like',"%$q%")->orWhere('title_hi','like',"%$q%")->orWhere('search_aliases','like',"%$q%")))->orderBy('sort_order')->latest()->paginate(12)->withQueryString(); return view('nani-dadi.category',compact('category','remedies','q')); }
    public function ingredient(string $slug, Request $request) {
        $ingredient = HomeRemedyIngredient::where('slug', $slug)->where('is_active', true)->firstOrFail();
        $q = trim((string) $request->query('q', ''));
        $remedies = $ingredient->remedies()->with(['category', 'ingredients'])->published()
            ->when($q, fn ($query) => $query->where(fn ($w) => $w->where('title_en', 'like', "%$q%")
                ->orWhere('title_hi', 'like', "%$q%")->orWhere('search_aliases', 'like', "%$q%")))
            ->orderBy('sort_order')->latest()->paginate(12)->withQueryString();
        return view('nani-dadi.ingredient', compact('ingredient', 'remedies', 'q'));
    }
