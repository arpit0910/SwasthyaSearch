...
function FeedbackForm() {
  ...
  async function submit(e) {
    ...
    try {
      ...
      setDone(t('Thank you. Your feedback helps improve Arogio.', 'धन्यवाद। आपकी प्रतिक्रिया Arogio को बेहतर बनाती है।'));
  ...
  const field = (name, label, type = 'text', extra = {}) => <TextField key={name} name={name} label={label} type={type} required fullWidth defaultValue={initial[name] || ''} error={Boolean(errors[name])} helperText={errors[name]?.[0]} slotProps={{ inputLabel: type === 'date' || type === 'time' ? { shrink: true } : undefined, htmlInput: { maxLength: name === 'comments' ? 2000 : 255 } }} {...extra} />;
  return <Box component="form" action={url('feedback.submit')} onSubmit={submit} method="post" sx={{ mt: 3 }}><input type="hidden" name="_token" value={config.csrf} />
    {done && <Alert severity="success" sx={{ mb: 2 }}>{done}</Alert>}{errors.form && <Alert severity="error" sx={{ mb: 2 }}>{errors.form[0]}</Alert>}
    <Box className="design-grid two" sx={{ gap: 2 }}>{field('name', t('Full name', 'पूरा नाम'))}{field('email', t('Email address', 'ईमेल'), 'email')}{field('category', t('Feedback category', 'प्रतिक्रिया श्रेणी'), 'text', { select: true, defaultValue: initial.category || 'General', children: ['General', 'Website Experience', 'Data Accuracy', 'Feature Request'].map(v => <MenuItem key={v} value={v}>{v}</MenuItem>) })}</Box>
    ...
    <Box sx={{ mt: 2 }}>{field('comments', t('Your feedback', 'आपकी प्रतिक्रिया'), 'text', { multiline: true, minRows: 3 })}</Box>
    <Button type="submit" variant="contained" disabled={busy} startIcon={busy ? <CircularProgress size={18} color="inherit" /> : <Check />} sx={{ mt: 2 }}>{t('Send feedback', 'प्रतिक्रिया भेजें')}</Button>
  </Box>;
}
function Home() {
  const [city] = useCity();
  ...
  return <Box component="main" id="main-content" tabIndex={-1} sx={{ bgcolor: 'background.default', color: 'text.primary' }}>
    ...
    <Container sx={{ py: 5 }}>
      ...
      <Box component="section" sx={{ my: 5 }}><Paper variant="outlined" sx={{ p: { xs: 3, md: 4 }, bgcolor: 'action.hover' }}><IconTile icon={People} /><Typography variant="h2" sx={{ mt: 2 }}>{t('Suggest a Missing Clinic or Provider', 'छूटे हुए क्लिनिक या प्रदाता को सुझाएँ')}</Typography><Typography color="text.secondary" sx={{ my: 2 }}>{t('Know a doctor or hospital that should be listed? Help your community discover more healthcare options.', 'क्या कोई डॉक्टर या अस्पताल सूची में नहीं है? अपने समुदाय को अधिक स्वास्थ्य विकल्प खोजने में मदद करें।')}</Typography><Stack direction={{ xs: 'column', md: 'row' }} gap={2} sx={{ my: 3 }}>{[t('Share the provider’s details', 'प्रदाता की जानकारी साझा करें'), t('Our team reviews the submission', 'हमारी टीम जानकारी की समीक्षा करती है'), t('Approved records join the directory', 'स्वीकृत रिकॉर्ड निर्देशिका में जुड़ते हैं')].map((label, i) => <Stack direction="row" gap={1.5} alignItems="center" key={label}><Chip label={i + 1} size="small" color="primary" /><Typography>{label}</Typography></Stack>)}</Stack><Button href={url('suggestions.create')} variant="outlined" endIcon={<Arrow />}>{t('Suggest a provider', 'प्रदाता सुझाएँ')}</Button></Paper></Box>
      ...
      <Accordion sx={{ mt: 5 }}><AccordionSummary expandIcon={<Down />}><Typography variant="h3">{t('Help us make Arogio better — share your feedback', 'Arogio को बेहतर बनाने में मदद करें — अपनी प्रतिक्रिया दें')}</Typography></AccordionSummary><AccordionDetails><FeedbackForm /></AccordionDetails></Accordion>
    </Container>
  </Box>;
}
function Footer() {
