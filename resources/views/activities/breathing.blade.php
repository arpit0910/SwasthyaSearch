...
@section('title', ($locale === 'hi' ? '????? ??????' : 'Breathing Exercise') . ' - Arogio')
...
<main class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
    ...
    <section class="breath-shell rounded-[2rem] border border-cyan-100/80 dark:border-slate-800 shadow-sm p-5 sm:p-8 lg:p-10">
        <div class="grid gap-8 xl:grid-cols-[0.95fr_1.05fr] xl:items-center">
            <div class="space-y-6">
                <div>
                    <p class="text-xs font-bold uppercase tracking-[0.24em] text-teal-700 dark:text-teal-300">{{ $isHindi ? '????? ????????' : 'Calm Breathing' }}</p>
                    <h1 class="mt-3 text-3xl sm:text-5xl font-extrabold tracking-tight text-slate-950 dark:text-white">{{ $isHindi ? '?? guided breathing session ?? ?? ??? ????? ??' : 'A guided breathing session that actually feels calming' }}</h1>
                    <p class="mt-4 text-sm sm:text-base leading-7 text-slate-600 dark:text-slate-300">{{ $isHindi ? '????? start ?????? ??? ???????, motion ?? voice cues ???? ????-???? inhale, hold ?? exhale ??? guide ??????? ?? flow ?? clutter ?? ?? pressure ?? ??? ????? ??? ???' : 'Just press start. The screen, motion, and voice cues then gently guide you through inhale, hold, and exhale with less clutter and less pressure.' }}</p>
                </div>

                <div class="grid gap-3 sm:grid-cols-3">
                    <div class="breath-panel rounded-[1.5rem] border border-white/80 dark:border-slate-700/80 p-4">
                        <p class="text-xs font-bold uppercase tracking-[0.2em] text-teal-700 dark:text-teal-300">{{ $isHindi ? 'Inhale' : 'Inhale' }}</p>
                        <p class="mt-2 text-2xl font-extrabold text-slate-950 dark:text-white">4s</p>
                        <p class="mt-1 text-sm text-slate-600 dark:text-slate-300">{{ $isHindi ? '??? ?? ???? ???? ???' : 'Slow breath in through the nose' }}</p>
                    </div>
                    <div class="breath-panel rounded-[1.5rem] border border-white/80 dark:border-slate-700/80 p-4">
                        <p class="text-xs font-bold uppercase tracking-[0.2em] text-sky-700 dark:text-sky-300">{{ $isHindi ? 'Hold' : 'Hold' }}</p>
                        <p class="mt-2 text-2xl font-extrabold text-slate-950 dark:text-white">4s</p>
                        <p class="mt-1 text-sm text-slate-600 dark:text-slate-300">{{ $isHindi ? '???? ?? ??? ????' : 'Keep the body soft' }}</p>
                    </div>
                    <div class="breath-panel rounded-[1.5rem] border border-white/80 dark:border-slate-700/80 p-4">
                        <p class="text-xs font-bold uppercase tracking-[0.2em] text-cyan-700 dark:text-cyan-300">{{ $isHindi ? 'Exhale' : 'Exhale' }}</p>
                        <p class="mt-2 text-2xl font-extrabold text-slate-950 dark:text-white">6s</p>
                        <p class="mt-1 text-sm text-slate-600 dark:text-slate-300">{{ $isHindi ? '???? ?? ???? release' : 'Long, slow release' }}</p>
                    </div>
                </div>
                ...
                <div class="breath-panel rounded-[1.6rem] border border-white/80 dark:border-slate-700/80 p-5">
                    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                        <div>
                            <p class="text-sm font-bold text-slate-950 dark:text-white">{{ $isHindi ? 'Session notes' : 'Session notes' }}</p>
                            <p id="breathing-support-copy" class="mt-1 text-sm leading-6 text-slate-600 dark:text-slate-300">{{ $isHindi ? '??? ?????, ?????? ?? ?????? ???, ?? ??????? ???? ?? ??? ????' : 'If you feel dizzy, panicky, or uncomfortable, return to your normal breathing.' }}</p>
                        </div>
                        <div class="flex flex-wrap gap-3">
                            <button id="breathing-start" class="inline-flex items-center justify-center rounded-[1.25rem] bg-teal-600 px-7 py-4 text-base font-extrabold text-white shadow-lg shadow-teal-600/20 transition hover:-translate-y-0.5 hover:bg-teal-700">{{ $isHindi ? '?? ???? ????' : 'Start now' }}</button>
                            <button id="breathing-voice" class="rounded-[1.1rem] border border-slate-200 dark:border-slate-700 px-4 py-3 text-sm font-bold text-slate-800 dark:text-slate-100">{{ $isHindi ? 'Voice On' : 'Voice On' }}</button>
                            <button id="breathing-restart" class="hidden rounded-[1.1rem] border border-slate-200 dark:border-slate-700 px-4 py-3 text-sm font-bold text-slate-800 dark:text-slate-100">{{ $isHindi ? 'Restart' : 'Restart' }}</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-5">
                    ...
                    <div id="breathing-stage" class="breath-stage">
                        ...
                        <div class="relative z-10 px-8">
                            <div class="breath-center-copy">
                                <div id="breathing-phase-chip" class="breath-center-text inline-flex rounded-full bg-slate-950/26 px-4 py-2 text-xs font-bold uppercase tracking-[0.24em]">{{ $isHindi ? 'Ready' : 'Ready' }}</div>
                                <div id="breathing-phase" class="breath-center-text mt-4 text-4xl sm:text-5xl font-extrabold">{{ $isHindi ? '???? ????' : 'Start' }}</div>
                                <div id="breathing-instruction" class="breath-center-text mt-3 text-sm sm:text-base font-medium leading-7">{{ $isHindi ? '?? ????, ??? ???? ?? ??? start ??????' : 'Press start for one calm, guided breathing loop.' }}</div>
                            </div>
                            <div class="mt-6 flex items-center justify-center gap-3">
                                ...
                                <div id="breathing-cycle" class="rounded-full bg-slate-950/28 px-4 py-2 text-sm font-bold text-white shadow-sm">{{ $isHindi ? 'Round 0/5' : 'Round 0/5' }}</div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="grid gap-4 sm:grid-cols-3">
                    <div class="breath-panel rounded-[1.4rem] border border-white/80 dark:border-slate-700/80 p-4">
                        <p class="text-xs uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400">{{ $isHindi ? 'Now' : 'Now' }}</p>
                        <p id="breathing-status" class="mt-2 text-sm font-semibold leading-6 text-slate-900 dark:text-slate-100">{{ $isHindi ? '????? ???? ?? start ??????' : 'Press start when you feel ready.' }}</p>
                    </div>
                    <div class="breath-panel rounded-[1.4rem] border border-white/80 dark:border-slate-700/80 p-4">
                        <p class="text-xs uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400">{{ $isHindi ? 'Elapsed' : 'Elapsed' }}</p>
                        <p id="breathing-timer" class="mt-2 text-2xl font-extrabold text-slate-950 dark:text-white">00:00</p>
                    </div>
                    <div class="breath-panel rounded-[1.4rem] border border-white/80 dark:border-slate-700/80 p-4">
                        <p class="text-xs uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400">{{ $isHindi ? 'Benefit' : 'Benefit' }}</p>
                        <p id="breathing-benefit" class="mt-2 text-sm font-semibold leading-6 text-slate-900 dark:text-slate-100">{{ $isHindi ? '???? exhale nervous system ?? soften ???? ???' : 'A longer exhale helps soften the nervous system.' }}</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    const phases = [
        {
            key: 'inhale',
            seconds: 4,
            label: isHindi ? 'Inhale' : 'Inhale',
            chip: isHindi ? 'Inhale' : 'Inhale',
            instruction: isHindi ? '??? ?? ????-???? ???? ???? ????' : 'Breathe in slowly through your nose.',
            status: isHindi ? '???? ????? ???? ???? ?????' : 'Fill the breath gently. Keep your shoulders soft.',
            benefit: isHindi ? 'Inhale ???? ?? grounded feel ????? ???' : 'Inhale helps you feel grounded and present.',
            scale: 1.04,
        },
        {
            key: 'hold',
            seconds: 4,
            label: isHindi ? 'Hold' : 'Hold',
            chip: isHindi ? 'Hold' : 'Hold',
            instruction: isHindi ? '???? ?????, ????? ???? ??? ???? ? ?????' : 'Hold the breath without tightening the body.',
            status: isHindi ? '?????? ????? ?? ???? ???? ?????' : 'Pause. Relax your jaw and forehead.',
            benefit: isHindi ? 'Short hold ????? ?? ???-??? ???? ?? ????? ???' : 'A short hold helps your attention settle.',
            scale: 1.08,
        },
        {
            key: 'exhale',
            seconds: 6,
            label: isHindi ? 'Exhale' : 'Exhale',
            chip: isHindi ? 'Exhale' : 'Exhale',
            instruction: isHindi ? '????-???? ???? ???? ???? ???????' : 'Let the breath out slowly and completely.',
            status: isHindi ? '??????? ??? ?????? ???? ?? ?????? calm ???? ???' : 'Release slowly. This is the part that calms the body most.',
            benefit: isHindi ? '???? exhale stress response ?? ???? ?? ???? ???' : 'A longer exhale can slow the stress response.',
            scale: 0.92,
        },
    ];

    const texts = {
        readyPhase: isHindi ? '???? ????' : 'Start',
        readyChip: 'Ready',
        readyInstruction: isHindi ? '?? ????, ??? ???? ?? ??? start ??????' : 'Press start for one calm, guided breathing loop.',
        readyStatus: isHindi ? '????? ???? ?? start ??????' : 'Press start when you feel ready.',
        readyBenefit: isHindi ? '???? exhale nervous system ?? soften ???? ???' : 'A longer exhale helps soften the nervous system.',
        completePhase: isHindi ? '????? ????' : 'Well done',
        completeInstruction: isHindi ? 'Session ???? ???? ??????? ???? ?? ????? ?? ????? ?? ???? ????? ????? ?? ??? ?? ?? ?????' : 'The session is complete. Return to your normal breath and notice whether your body feels even a little lighter.',
        completeStatus: isHindi ? '???? 5 guided rounds ???? ?? ????' : 'You completed 5 guided rounds.',
        completeBenefit: isHindi ? '???? calm breaks ??? ?? ??? ???-??? ??? ???? ????' : 'Small calm breaks help most when repeated through the day.',
        voiceOn: 'Voice On',
        voiceOff: 'Voice Off',
        supportDefault: isHindi ? '??? ?????, ?????? ?? ?????? ???, ?? ??????? ???? ?? ??? ????' : 'If you feel dizzy, panicky, or uncomfortable, return to your normal breathing.',
        supportComplete: isHindi ? '??? ?????, ?? ?? round ?? ???? ??? — ????? ???? pressure ???' : 'If you want, you can do another round — without pressure.',
        roundLabel: 'Round',
    };
