...
<main class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
    <section class="rounded-[2rem] border border-cyan-100/80 dark:border-slate-800 bg-white/90 dark:bg-slate-900/90 shadow-sm p-5 sm:p-8 lg:p-10">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            ...
            <button id="memory-reset" type="button" class="rounded-[1.15rem] bg-cyan-600 hover:bg-cyan-700 px-6 py-3 text-sm font-bold text-white">{{ $isHindi ? 'फिर से चालू करें' : 'Play again' }}</button>
        </div>
        ...
        <div class="mt-8 grid gap-4 sm:grid-cols-4">
            ...
            <div class="rounded-[1.4rem] bg-sky-50 dark:bg-sky-950/30 p-4">
                <p class="text-xs uppercase tracking-[0.2em] text-sky-700 dark:text-sky-300">{{ $isHindi ? 'मैच' : 'Matches' }}</p>
            ...
            <div class="rounded-[1.4rem] bg-violet-50 dark:bg-violet-950/30 p-4">
                <p class="text-xs uppercase tracking-[0.2em] text-violet-700 dark:text-violet-300">{{ $isHindi ? 'लेवल' : 'Level' }}</p>
...
(() => {
    const levelConfigs = [
        { level: 1, tiles: 8, cols: 'grid-cols-4' },
        { level: 2, tiles: 10, cols: 'grid-cols-5' },
        { level: 3, tiles: 12, cols: 'grid-cols-4 sm:grid-cols-6' },
        { level: 4, tiles: 14, cols: 'grid-cols-4 sm:grid-cols-7' },
        { level: 5, tiles: 16, cols: 'grid-cols-4' },
        { level: 6, tiles: 20, cols: 'grid-cols-4 sm:grid-cols-5' },
    ];

    const tileIcons = ['heart', 'star', 'sun', 'moon-star', 'cloud', 'leaf', 'flame', 'sparkles', 'pill', 'apple', 'flower-2', 'umbrella', 'bird', 'trees'];
    ...
    const buildDeck = () => {
        ...
        return shuffle([...selectedIcons, ...selectedIcons]).map((value, index) => ({ value, index, done: false }));
    };
    ...
    const renderLevels = () => {
        ...
        levelConfigs.forEach((config, index) => {
            ...
            button.type = 'button';
            button.className = `rounded-full border px-3 py-2 text-xs font-bold transition ${index === currentLevelIndex ? 'border-violet-500 bg-violet-600 text-white shadow-lg shadow-violet-500/20' : 'border-slate-200 bg-white text-slate-700 hover:border-violet-300 hover:text-violet-700 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200 dark:hover:border-violet-500 dark:hover:text-violet-300'}`;
    ...
    const renderCardBack = (button) => {
        button.innerHTML = '<span class="flex h-full w-full items-center justify-center rounded-[1.3rem] bg-white/60 dark:bg-slate-900/40"><i data-lucide="help-circle" class="h-8 w-8 text-cyan-600 dark:text-cyan-300"></i></span>';
    };

    const renderCardFront = (button, iconName) => {
        button.innerHTML = `<span class="flex h-full w-full items-center justify-center rounded-[1.3rem] bg-cyan-100 text-cyan-700 dark:bg-cyan-950/40 dark:text-cyan-200"><i data-lucide="${iconName}" class="h-8 w-8 sm:h-9 sm:w-9"></i></span>`;
    };
