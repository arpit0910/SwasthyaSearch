    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow">
            <div class="modal-header bg-light border-0">
                <h5 class="modal-title fw-bold text-dark" id="syncModalLabel"><i class="fa-solid fa-rotate me-2 text-primary"></i>Sync City Doctors Directory</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body p-4">
                <!-- Input Form Section -->
                <div id="syncFormSection">
                    <p class="text-muted mb-4">Enter the city name to live-scrape and synchronize verified doctors across 18 medical specialties. Existing records will be updated automatically.</p>
                    <form id="syncDoctorsForm">
                        <div class="mb-4">
                            ...
                            <div class="form-text">The scraper will fetch directly from public directories for this city.</div>
                        </div>
                ...
                <div id="syncProgressSection" style="display: none;" class="py-3 text-center">
                    <div class="mb-3">
                        ...
                        <p id="syncStatusMessage" class="text-muted fs-7 mb-4">Connecting to medical directories...</p>
                    </div>
                    <div class="progress mb-3" style="height: 24px; border-radius: 12px;">
                        <div id="syncProgressBar" class="progress-bar progress-bar-striped progress-bar-animated bg-primary fw-bold fs-7" role="progressbar" style="width: 5%;" aria-valuenow="5" aria-valuemin="0" aria-valuemax="100">5%</div>
                    </div>
...
document.addEventListener('DOMContentLoaded', function () {
    const syncForm = document.getElementById('syncDoctorsForm');
    if (!syncForm) return;
    ...
    syncForm.addEventListener('submit', function (e) {
        ...
        // Start Sync Request
        fetch('{{ route('admin.doctors.sync') }}', {
            method: 'POST',
            headers: {
        ...
        const interval = setInterval(() => {
            fetch('{{ route('admin.doctors.sync.progress') }}')
            .then(res => res.json())
            .then(data => {
                if (data.progress) {
