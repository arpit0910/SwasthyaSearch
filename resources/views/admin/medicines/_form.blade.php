...
$faqLines = $medicine && is_array($medicine->faqs_json ?? null)
    ? collect($medicine->faqs_json)->map(fn ($faq) => trim(($faq['question'] ?? '') . ' | ' . ($faq['answer'] ?? '')))->implode("\n")
    : '';
$sourceLines = $medicine && is_array($medicine->source_references_json ?? null)
    ? collect($medicine->source_references_json)->implode("\n")
    : '';
$brandLines = $medicine && is_array($medicine->brand_names_json ?? null)
...
<div class="card-body p-4">
    <div class="row g-3">
        <div class="col-md-6">
            ...
            <input type="text" name="name" class="form-control" required value="{{ old('name', $medicine->name ?? '') }}">
        </div>
        <div class="col-md-6">
            <label class="form-label fw-semibold">Slug</label>
            <input type="text" name="slug" class="form-control" value="{{ old('slug', $medicine->slug ?? '') }}" placeholder="leave blank to auto-generate">
        </div>
        <div class="col-md-4">
        ...
        <div class="col-md-4">
            ...
            <input type="text" name="composition" class="form-control" value="{{ old('composition', $medicine->composition ?? '') }}">
        </div>
        <div class="col-md-4">
        ...
        <div class="col-md-4">
            ...
            <input type="text" name="medicine_type" class="form-control" value="{{ old('medicine_type', $medicine->medicine_type ?? '') }}" placeholder="Tablet / Syrup">
        </div>
        <div class="col-md-4">
        ...
        <div class="col-md-4">
            ...
            <select name="review_status" class="form-select" required>
                @foreach(\App\Models\Medicine::REVIEW_STATUSES as $status)
                    <option value="{{ $status }}" @selected(old('review_status', $medicine->review_status ?? 'draft') === $status)>{{ ucfirst(str_replace('_', ' ', $status)) }}</option>
                @endforeach
            </select>
        </div>
        <div class="col-12">
            ...
            <div class="row g-3">
                <div class="col-md-4">
                    <div class="form-check mt-2">
                        <input class="form-check-input" type="checkbox" name="prescription_required" value="1" id="prescription_required" @checked(old('prescription_required', $medicine->prescription_required ?? false))>
                        <label class="form-check-label" for="prescription_required">Prescription required</label>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="form-check mt-2">
                        <input class="form-check-input" type="checkbox" name="medically_reviewed" value="1" id="medically_reviewed" @checked(old('medically_reviewed', $medicine->medically_reviewed ?? false))>
                        <label class="form-check-label" for="medically_reviewed">Medically reviewed</label>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="form-check mt-2">
                        <input class="form-check-input" type="checkbox" name="is_published" value="1" id="is_published" @checked(old('is_published', $medicine->is_published ?? false))>
        ...
        @foreach ([
            ...
            <div class="col-md-6">
                ...
                <textarea name="{{ $field }}_en" class="form-control" rows="3">{{ old($field . '_en', $medicine?->{$field . '_en'} ?? '') }}</textarea>
            </div>
            <div class="col-md-6">
                <label class="form-label fw-semibold">{{ $label }} (Hindi)</label>
                <textarea name="{{ $field }}_hi" class="form-control" rows="3">{{ old($field . '_hi', $medicine?->{$field . '_hi'} ?? '') }}</textarea>
            </div>
        @endforeach

        <div class="col-md-6">
            <label class="form-label fw-semibold">Meta Title (English)</label>
            <input type="text" name="meta_title_en" class="form-control" value="{{ old('meta_title_en', $medicine->meta_title_en ?? '') }}">
        </div>
        <div class="col-md-6">
            <label class="form-label fw-semibold">Meta Title (Hindi)</label>
            <input type="text" name="meta_title_hi" class="form-control" value="{{ old('meta_title_hi', $medicine->meta_title_hi ?? '') }}">
        </div>
        <div class="col-md-6">
            <label class="form-label fw-semibold">Meta Description (English)</label>
            <textarea name="meta_description_en" class="form-control" rows="2">{{ old('meta_description_en', $medicine->meta_description_en ?? '') }}</textarea>
        </div>
        <div class="col-md-6">
            <label class="form-label fw-semibold">Meta Description (Hindi)</label>
            <textarea name="meta_description_hi" class="form-control" rows="2">{{ old('meta_description_hi', $medicine->meta_description_hi ?? '') }}</textarea>
        </div>
        <div class="col-md-6">
            <label class="form-label fw-semibold">Reviewed By</label>
            <input type="text" name="reviewed_by" class="form-control" value="{{ old('reviewed_by', $medicine->reviewed_by ?? '') }}">
        </div>
        <div class="col-md-6">
            <label class="form-label fw-semibold">Last Reviewed At</label>
            <input type="date" name="last_reviewed_at" class="form-control" value="{{ old('last_reviewed_at', optional($medicine->last_reviewed_at ?? null)->toDateString()) }}">
        </div>
        <div class="col-12">
