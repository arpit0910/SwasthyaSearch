...
@php
    ...
    $hasActiveMobileFilters =
        !empty(array_filter((array) request('blood_group', []))) ||
...
@section('content')
    <!-- Hero Section -->
    <header
        class="bg-gradient-to-r from-rose-600 via-rose-700 to-red-700 dark:from-slate-900 dark:via-red-900 dark:to-slate-900 text-white py-16 px-4 sm:px-6 lg:px-8 border-b border-rose-800 dark:border-slate-700 ring-1 ring-black/10 dark:ring-white/15 shadow-xl dark:shadow-black/50 relative overflow-hidden">
        <div
            class="absolute inset-0 bg-[radial-gradient(circle_at_top_right,rgba(251,113,133,0.18),transparent_55%)] dark:opacity-0">
        </div>
        <!-- Glowing background blobs -->
        <div class="glow-blob w-[300px] h-[300px] bg-rose-300/15 dark:opacity-0 top-0 left-0 absolute rounded-full blur-3xl">
        </div>
        <div
            class="glow-blob w-[400px] h-[400px] bg-red-300/15 dark:opacity-0 bottom-0 right-0 absolute rounded-full blur-3xl">
        </div>
        <div class="max-w-7xl mx-auto text-center relative z-10">
            <span
                class="bg-white/10 text-rose-100 border border-white/25 px-4 py-1.5 rounded-full text-xs font-semibold tracking-wider uppercase inline-block mb-4 shadow-sm backdrop-blur-sm">
                {{ $locale === 'hi' ? 'सत्यापित रक्त केंद्र' : 'Verified Blood Centers' }}
            </span>
            <h1
                class="text-4xl sm:text-5xl font-extrabold tracking-tight mb-4 bg-gradient-to-r from-white via-slate-100 to-slate-300 bg-clip-text text-transparent py-2 leading-tight">
    ...
    <section class="lg:hidden max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-4 relative z-20 w-full mb-6">
        <form action="{{ route('blood_banks.index') }}" method="POST"
            class="bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200/80 dark:border-slate-700/70 p-4 sm:p-5 backdrop-blur-xl">
            @csrf
            ...
            <div class="relative">
                <i data-lucide="search"
                    class="absolute inset-y-0 left-0 flex items-center pl-4 pointer-events-none w-4 h-4 text-slate-400"></i>
                ...
                <button type="button" data-open-mobile-filters onclick="openMobileFilters()" aria-label="Open filters"
                    class="lg:hidden absolute right-1.5 top-1/2 -translate-y-1/2 w-9 h-9 rounded-lg border flex items-center justify-center transition-all duration-200 {{ $hasActiveMobileFilters ? 'border-red-600 bg-red-600 text-white shadow-md shadow-red-500/30' : 'border-red-200 bg-white text-red-700 hover:bg-red-50' }}"
                    title="Filters">
                    <i data-lucide="filter" class="w-4 h-4"></i>
                </button>
            </div>
            <button type="button" onclick="toggleNearby(this)" data-nearby-toggle data-nearby-theme="red"
                class="mt-3 w-full h-11 px-4 rounded-xl border font-bold text-sm transition-all duration-200 flex items-center justify-center space-x-2 shadow-2xs {{ $isNearbyActive ? 'border-red-600 dark:border-red-500 bg-red-600 dark:bg-red-600 text-white' : 'border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-950/35 hover:bg-red-100 dark:hover:bg-red-900/40 text-red-700 dark:text-red-300' }}">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-20 w-full mb-2 lg:mb-8">
        <div id="mobile-filter-sidebar" class="fixed inset-0 z-[120] hidden lg:hidden" aria-hidden="true">
            <div id="mobile-filter-backdrop" onclick="closeMobileFilters()"
            ...
            <div id="mobile-filter-drawer"
                class="absolute top-0 right-0 h-full w-full max-w-sm bg-white dark:bg-slate-900 shadow-2xl overflow-y-auto transform translate-x-full transition-transform duration-300 ease-out">
                <div
                    class="sticky top-0 p-3 sm:p-4 border-b border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 flex items-center justify-between">
                    <h2 class="text-lg font-bold text-slate-900 dark:text-slate-100">
                        {{ $locale === 'hi' ? 'फ़िल्टर' : 'Filters' }}</h2>
                    <button type="button" id="mobile-filter-close" onclick="closeMobileFilters()"
                        aria-label="Close filters"
                        class="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors">
                        <i data-lucide="x" class="w-5 h-5 text-slate-600 dark:text-slate-300"></i>
                    </button>
                </div>
                <form action="{{ route('blood_banks.index') }}" method="POST" class="p-3 sm:p-4 space-y-4"
                    id="mobile-filter-form">
                    @csrf
                    <input type="hidden" name="search" value="{{ request('search', $filters['search'] ?? '') }}">
                    <input type="hidden" name="user_lat" id="user_lat_mobile"
                        value="{{ request('user_lat', $filters['user_lat'] ?? '') }}">
                    <input type="hidden" name="user_lng" id="user_lng_mobile"
                        value="{{ request('user_lng', $filters['user_lng'] ?? '') }}">
                    <div>
                        <label
                            class="block text-xs font-bold text-slate-600 mb-2 uppercase tracking-wider">{{ $locale === 'hi' ? 'रक्त समूह' : 'Blood Group' }}</label>
                        ...
                        <select name="blood_group[]" multiple
                            ...
                            @foreach ($bloodGroups as $bg)
                                <option value="{{ $bg }}" {{ in_array($bg, $selectedBG) ? 'selected' : '' }}>
                                    {{ $bg }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="hidden">
                        <label
                            class="block text-xs font-bold text-slate-600 mb-2 uppercase tracking-wider">{{ $locale === 'hi' ? 'सुविधाएं' : 'Facilities' }}</label>
                        @php $selectedFac = is_array(request('facility')) ? request('facility') : (request('facility') && request('facility') !== 'All' ? [request('facility')] : []); @endphp
                        <select name="facility[]" multiple
                            data-placeholder="{{ $locale === 'hi' ? 'सुविधा चुनें' : 'Select facilities' }}"
                            class="h-12 w-full px-4 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-red-500 focus:ring-2 focus:ring-red-500/20 transition-all duration-200 font-medium text-slate-700">
                            <option value="24x7" {{ in_array('24x7', $selectedFac) ? 'selected' : '' }}>
                                {{ $locale === 'hi' ? '24x7 उपलब्ध' : '24x7 Available' }}</option>
                            <option value="Government" {{ in_array('Government', $selectedFac) ? 'selected' : '' }}>
                                {{ $locale === 'hi' ? 'सरकारी ब्लड बैंक' : 'Government Blood Bank' }}</option>
                            <option value="Private" {{ in_array('Private', $selectedFac) ? 'selected' : '' }}>
                                {{ $locale === 'hi' ? 'निजी ब्लड बैंक' : 'Private Blood Bank' }}</option>
                            <option value="Component" {{ in_array('Component', $selectedFac) ? 'selected' : '' }}>
                                {{ $locale === 'hi' ? 'रक्त घटक' : 'Blood Components' }}</option>
                            <option value="Apheresis" {{ in_array('Apheresis', $selectedFac) ? 'selected' : '' }}>
                                {{ $locale === 'hi' ? 'एफेरेसिस' : 'Apheresis (SDP)' }}</option>
                        </select>
                    </div>
                    <div>
                        <label
                            class="block text-xs font-bold text-slate-600 mb-2 uppercase tracking-wider">{{ $locale === 'hi' ? 'शहर' : 'City' }}</label>
                        <select name="city[]"
                            class="h-12 w-full px-4 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-red-500 focus:ring-2 focus:ring-red-500/20 transition-all duration-200 font-medium text-slate-700">
                    ...
                    <div class="space-y-2.5 pt-3 border-t border-slate-200">
                        <button type="submit"
                            class="w-full h-11 px-4 rounded-xl border border-red-300 bg-red-600 hover:bg-red-700 text-white font-bold text-sm transition-all duration-200 flex items-center justify-center space-x-2 shadow-2xs">
                            <i data-lucide="filter" class="w-4 h-4"></i>
                            <span>{{ $locale === 'hi' ? 'फ़िल्टर लागू करें' : 'Apply Filters' }}</span>
                        </button>
                        <a href="{{ route('blood_banks.index') }}"
                            class="w-full h-11 px-4 rounded-xl border border-slate-200 hover:border-slate-300 text-slate-600 hover:text-slate-800 hover:bg-slate-50 font-bold text-sm transition-all duration-200 flex items-center justify-center space-x-2 shadow-2xs">
    ...
    <section id="filter-panel" class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-20 w-full mb-2 hidden lg:block">
        <form action="{{ route('blood_banks.index') }}" method="POST"
            class="bg-white rounded-2xl shadow-xl border border-slate-200/80 ring-1 ring-slate-200/70 p-5 sm:p-6 backdrop-blur-xl"
            id="filter-form">
            @csrf
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 items-stretch">
                ...
                <div>
                    @php
                        $selectedBG = is_array(request('blood_group'))
                            ? request('blood_group')
                            : (request('blood_group') && request('blood_group') !== 'All'
                                ? [request('blood_group')]
                                : []);
                    @endphp
                    <select name="blood_group[]" multiple
                ...
                <div>
                    @php
                        $selectedFac = is_array(request('facility'))
                            ? request('facility')
                            : (request('facility') && request('facility') !== 'All'
                                ? [request('facility')]
                                : []);
                    @endphp
                    <select name="facility[]" multiple
                        ...
                        <option value="Private" {{ in_array('Private', $selectedFac) ? 'selected' : '' }}>
                            {{ $locale === 'hi' ? 'निजी ब्लड बैंक' : 'Private Blood Bank' }}</option>
                        <option value="Component" {{ in_array('Component', $selectedFac) ? 'selected' : '' }}>
                            {{ $locale === 'hi' ? 'रक्त घटक (प्लेटलेट्स आदि)' : 'Blood Components' }}</option>
                        <option value="Apheresis" {{ in_array('Apheresis', $selectedFac) ? 'selected' : '' }}>
                            {{ $locale === 'hi' ? 'एफेरेसिस (एसडीपी)' : 'Apheresis (SDP)' }}</option>
                    </select>
                </div>
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex-1 w-full mt-0 lg:mt-4 pb-20">
        @if (count($bloodBanks) === 0)
            <div class="bg-white rounded-3xl border border-slate-200/80 p-16 text-center shadow-sm max-w-2xl mx-auto">
                <div
                    class="w-20 h-20 bg-red-50 rounded-full flex items-center justify-center mx-auto mb-6 text-red-600 border border-red-100 shadow-inner">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                @foreach ($bloodBanks as $bankObj)
                    @php
                        ...
                        $addrEn = is_array($bank->address)
                            ? $bank->address['en']
                            : $bank->address['en'] ?? $bank->address_en;
                        $addrHi = is_array($bank->address)
                            ? $bank->address['hi']
                            : $bank->address['hi'] ?? $bank->address_hi;
                    ...
                    <div class="glass-card rounded-3xl border border-slate-200/80 dark:border-slate-800 shadow-2xs hover:shadow-xl transition-all duration-300 overflow-hidden flex flex-col group hover:-translate-y-1 relative"
                        ...
                        <!-- Card Header -->
                        <div
                            class="p-6 pb-4 bg-gradient-to-br from-slate-50/50 via-white/50 to-slate-50/50 dark:from-slate-800/30 dark:via-transparent dark:to-slate-800/30 border-b border-slate-100 dark:border-slate-800/60 flex items-start space-x-4">
                            <div
                                class="w-10 h-10 bg-gradient-to-tr from-red-500 to-rose-600 rounded-xl p-0.5 shadow-md shrink-0 group-hover:scale-105 transition-transform duration-300">
                                <div
                                    class="w-full h-full bg-slate-900 rounded-[10px] flex items-center justify-center text-white font-extrabold text-sm tracking-wider">
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex items-start justify-between gap-2 mb-1">
                                    <h3
                                        class="font-extrabold text-lg text-slate-900 dark:text-white line-clamp-3 leading-snug group-hover:text-red-600 transition-colors duration-200">
                                        {{ $bName }}
                                    </h3>
                                    <div class="shrink-0 flex items-center gap-1.5 mt-0.5 relative">
                                        ...
                                        <div id="blood_bank-actions-{{ $bank->id }}"
                                            class="listing-action-menu hidden absolute right-0 top-10 z-20 w-52 rounded-2xl border border-slate-200/90 dark:border-slate-700 bg-white/95 dark:bg-slate-900/95 shadow-xl p-2 backdrop-blur">
                                            <button type="button"
                                                ...
                                                <i data-lucide="thumbs-up"
                                                    class="w-4 h-4 text-emerald-600 dark:text-emerald-300"></i>
                                            ...
                                            <button type="button"
                                                ...
                                                <i data-lucide="triangle-alert"
                                                    class="w-4 h-4 text-rose-600 dark:text-rose-300"></i>
                                ...
                                <div class="flex flex-wrap gap-1 mb-2">
                                    @if ($bank->distance_km !== null)
                                        <span
                                            class="text-[9px] font-bold uppercase tracking-wider text-emerald-700 dark:text-emerald-300 bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-100 dark:border-emerald-900/60 px-1.5 py-0.5 rounded-md shadow-2xs">
                                            {{ $bank->distance_km }} km {{ $locale === 'hi' ? 'दूर' : 'away' }}
                                        </span>
                                    @endif
                                    @if ($bank->is_24_7)
                                        <span
                                            class="text-[9px] font-bold uppercase tracking-wider text-emerald-600 dark:text-emerald-300 bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-100 dark:border-emerald-900/60 px-1.5 py-0.5 rounded-md shadow-2xs">
                                            {{ $locale === 'hi' ? '24x7 उपलब्ध' : '24x7 Open' }}
                                        </span>
                                    @endif
                                    @if ($bank->is_government)
                                        <span
                                            class="text-[9px] font-bold uppercase tracking-wider text-cyan-600 dark:text-indigo-300 bg-cyan-50 dark:bg-indigo-950/40 border border-cyan-100 dark:border-indigo-900/60 px-1.5 py-0.5 rounded-md shadow-2xs">
                                            {{ $locale === 'hi' ? 'सरकारी' : 'Govt' }}
                                        </span>
                                    @else
                                        <span
                                            class="text-[9px] font-bold uppercase tracking-wider text-amber-600 dark:text-amber-300 bg-amber-50 dark:bg-amber-950/40 border border-amber-100 dark:border-amber-900/60 px-1.5 py-0.5 rounded-md shadow-2xs">
                        ...
                        <div class="p-6 flex-1 flex flex-col space-y-4">
                            <div
                                class="bg-slate-50 dark:bg-slate-800/40 p-4 rounded-2xl border border-slate-100/80 dark:border-slate-700/80 space-y-3 shadow-2xs">
                                <h4
                                    class="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider flex items-center space-x-1.5">
                                ...
                                <div class="flex flex-wrap gap-1.5 pt-1">
                                    @if (!empty($bank->available_blood_groups) && count($bank->available_blood_groups) > 0)
                                        @foreach ($bank->available_blood_groups as $bg)
                                            <span
                                                class="bg-red-50 dark:bg-red-950/40 text-red-700 dark:text-red-300 border border-red-200 dark:border-red-900/60 px-2.5 py-1 rounded-xl text-xs font-extrabold shadow-2xs flex items-center space-x-1">
                                    ...
                                    @else
                                        <span
                                            class="text-xs text-slate-400 dark:text-slate-500 italic">{{ $locale === 'hi' ? 'जानकारी उपलब्ध नहीं' : 'Stock info unavailable' }}</span>
                                    @endif
                                </div>
                                <p
                                    class="text-[11px] text-amber-800 dark:text-amber-300 bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-900/60 rounded-xl px-2.5 py-1.5">
                            ...
                            <button type="button" onclick="toggleMobileAccordion('bank-{{ $bank->id }}')"
                                ...
                                <span class="flex items-center space-x-2">
                                    ...
                                    <span
                                        id="bank-{{ $bank->id }}-text">{{ $locale === 'hi' ? 'पता और सुविधाएं देखें' : 'View Address & Facilities' }}</span>
                                </span>
                                <i data-lucide="chevron-down" id="bank-{{ $bank->id }}-icon"
                            ...
                            <div id="bank-{{ $bank->id }}-content" class="hidden md:flex flex-col space-y-4 flex-1">
                                <div class="grid grid-cols-2 gap-2 text-xs font-semibold">
                                    <div
                                        class="bg-slate-50 dark:bg-slate-800/40 p-3 rounded-2xl border border-slate-100/80 dark:border-slate-700/80 flex items-center space-x-2 shadow-2xs">
                                        <i data-lucide="layers"
                                            class="w-4 h-4 {{ $bank->component_facility ? 'text-emerald-500' : 'text-slate-300' }} shrink-0"></i>
                                        <div class="truncate">
                                            <span
                                                class="text-slate-400 block text-[10px] uppercase">{{ $locale === 'hi' ? 'रक्त घटक' : 'Components' }}</span>
                                            <span
                                                class="{{ $bank->component_facility ? 'text-slate-900 dark:text-white font-bold' : 'text-slate-400 font-normal' }}">
                                    ...
                                    <div
                                        ...
                                        <i data-lucide="repeat"
                                            class="w-4 h-4 {{ $bank->apheresis_facility ? 'text-teal-500' : 'text-slate-300' }} shrink-0"></i>
                                        <div class="truncate">
                                            <span
                                                class="text-slate-400 block text-[10px] uppercase">{{ $locale === 'hi' ? 'एफेरेसिस (एसडीपी)' : 'Apheresis' }}</span>
                                            <span
                                                class="{{ $bank->apheresis_facility ? 'text-slate-900 dark:text-white font-bold' : 'text-slate-400 font-normal' }}">
                                        ...
                                        <div class="flex-1 space-y-1">
                                            <div>
                                                ...
                                                @if (!empty($bank->city) || !empty($bank->state))
                                                    <span
                                                        class="text-slate-500 text-[11px] block">{{ implode(', ', array_filter([$bank->city, $bank->state, $bank->pincode])) }}</span>
                                                @endif
                                            </div>
                        ...
                        <div class="p-6 pt-0 flex flex-col sm:flex-row gap-2.5 sm:gap-3">
                            ...
                            @endif
                            <a href="{{ !empty($bank->phone) ? 'tel:' . (($bank->country_code ?? '') . $bank->phone) : '#' }}"
                            ...
                            @if (!empty($bank->website))
                                <a href="{{ str_starts_with($bank->website, 'http') ? $bank->website : 'https://' . $bank->website }}"
                                    target="_blank"
...
@push('scripts')
    <script>
        function enhanceMultiSelectDropdown(selectEl) {
            ...
            trigger.type = 'button';
            trigger.className =
                'h-12 w-full px-4 bg-white border border-slate-200 rounded-xl text-sm font-medium text-slate-700 flex items-center justify-between';
            const placeholder = selectEl.dataset.placeholder || 'Select options';
            trigger.innerHTML =
                `<span class="multi-select-label truncate text-left">${placeholder}</span><i data-lucide="chevron-down" class="w-4 h-4 text-slate-500"></i>`;

            const panel = document.createElement('div');
            panel.className =
                'hidden absolute z-50 mt-2 w-full max-h-64 overflow-auto rounded-xl border border-slate-200 bg-white shadow-lg p-2 space-y-1';
        ...
        function renderMultiSelectBadges(selectEl) {
            ...
            if (!badgeWrap.dataset.removeBound) {
                badgeWrap.addEventListener('click', (e) => {
                    ...
                    const panelBtn = selectEl.parentElement.querySelector(
                        `.multi-select-dropdown button[data-index="${Array.from(selectEl.options).indexOf(option)}"]`
                        );
                    if (panelBtn) {
                        panelBtn.className =
                            'w-full text-left px-3 py-2 rounded-md cursor-pointer text-sm transition-colors text-slate-700 hover:bg-slate-50';
                    }
                    selectEl.dispatchEvent(new Event('change', {
                        bubbles: true
                    }));
        ...
        function setNearbyButtonState(button, isActive) {
            ...
            const inactiveClasses = ['border-red-200', 'dark:border-red-800', 'bg-red-50', 'dark:bg-red-950/35',
        ...
        function setUserLocationAndSubmit(form, button = null) {
            ...
            navigator.geolocation.getCurrentPosition(function(position) {
                const locationApplied = applyUserLocationToForm(form, position.coords.latitude.toFixed(6), position
        ...
        function toggleClientFilter(type) {
            ...
            if (btn) {
                if (activeClientFilters[type]) {
                    btn.classList.add('bg-red-50', 'dark:bg-red-950/40', 'border-red-500', 'text-red-700',
                        'dark:text-red-300');
                    btn.classList.remove('bg-white', 'dark:bg-slate-800', 'border-slate-200', 'dark:border-slate-700',
                        'text-slate-700', 'dark:text-slate-350');
                } else {
                    btn.classList.remove('bg-red-50', 'dark:bg-red-950/40', 'border-red-500', 'text-red-700',
                        'dark:text-red-300');
