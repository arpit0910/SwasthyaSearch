...
@section('title', ($locale === 'hi' ? 'अस्पताल व क्लीनिक' : 'Hospitals & Clinics') . ' - Arogio')

@php
$seoCityInput = request('city');
$seoCity = $activeCity ?? config('healthcare.active_city', 'Jaipur');
$hasCity = true;
$pageTitle = "Hospitals & Clinics | Find Services & Contacts | Arogio";
...
: 'Search hospitals by department, service, or locality. Find hospital contact details, locations, and direct access to healthcare providers.';
...
<header
    ...
    <div class="max-w-7xl mx-auto text-center relative z-10">
        ...
        <h1
            ...
            {{ $locale === 'hi' ? 'अस्पताल और क्लीनिक खोजें' : 'Find Hospitals & Clinics' }}
        </h1>
...
<section class="lg:hidden max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-4 relative z-20 w-full mb-6">
    <form action="{{ route('hospitals.index') }}" method="POST" class="bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200/80 dark:border-slate-700/70 p-4 sm:p-5 backdrop-blur-xl">
        ...
        <div class="relative">
            ...
            <button type="button" data-open-mobile-filters onclick="openMobileFilters()" aria-label="Open filters" class="lg:hidden absolute right-1.5 top-1/2 -translate-y-1/2 w-9 h-9 rounded-lg border flex items-center justify-center transition-all duration-200 {{ $hasActiveMobileFilters ? 'border-teal-600 bg-teal-600 text-white shadow-md shadow-teal-500/30' : 'border-teal-200 bg-white text-teal-700 hover:bg-teal-50' }}" title="Filters">
    ...
    <form action="{{ route('hospitals.index') }}" method="POST"
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 items-stretch">
            ...
            <div class="relative">
                <i data-lucide="search" class="absolute inset-y-0 left-0 flex items-center pl-4 pointer-events-none w-4 h-4 text-slate-400"></i>
                <input type="text" name="search"
                    placeholder="{{ $locale === 'hi' ? 'अस्पताल, क्लिनिक, पता या शहर खोजें...' : 'Search hospitals, clinics, address, or services...' }}"
                    value="{{ request('search', $filters['search'] ?? '') }}"
                    class="h-12 w-full pl-10 pr-4 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-teal-500 focus:ring-2 focus:ring-teal-500/20 transition-all duration-200 font-medium" />
            </div>
                ...
                <div class="flex-1 min-w-0">
                    <div class="flex items-start justify-between gap-2 mb-1.5">
                        ...
                        <div class="shrink-0 flex items-center gap-1.5 mt-0.5 relative">
                            ...
                            <button
                                type="button"
                                onclick="toggleListingActionMenu(event, 'hospital-actions-{{ $h->id }}')"
                            ...
                            <div id="hospital-actions-{{ $h->id }}" class="listing-action-menu hidden absolute right-0 top-10 z-20 w-52 rounded-2xl border border-slate-200/90 dark:border-slate-700 bg-white/95 dark:bg-slate-900/95 shadow-xl p-2 backdrop-blur">
                                <button
                                    type="button"
                                    onclick="submitUsefulVoteAndClose('hospital', {{ $h->id }}, @js($hName), 'hospital-actions-{{ $h->id }}')"
                                ...
                                <button
                                    type="button"
                                    onclick="openListingReportFromMenu('hospital', {{ $h->id }}, @js($hName), 'hospital-actions-{{ $h->id }}')"
                ...
                <div id="hosp-{{ $h->id }}-content" class="hidden md:flex flex-col space-y-5 flex-1">
                    ...
                    <div class="space-y-2 pt-2 border-t border-slate-100 dark:border-slate-800">
                        ...
                        <div class="flex flex-wrap gap-2 overflow-hidden">
                            ...
                            <span
                                class="min-w-0 max-w-full bg-emerald-50 dark:bg-emerald-900/30 text-emerald-800 dark:text-emerald-200 border border-emerald-200 dark:border-emerald-700/70 px-2.5 py-1 rounded-lg text-xs font-bold inline-flex items-center gap-1.5 shadow-2xs">
                            ...
                            <span
                                class="min-w-0 max-w-full bg-sky-50 dark:bg-sky-900/30 text-sky-800 dark:text-sky-200 border border-sky-200 dark:border-sky-700/70 px-2.5 py-1 rounded-lg text-xs font-bold inline-flex items-center gap-1.5 shadow-2xs">
                            ...
                            <span
                                class="min-w-0 max-w-full bg-teal-50 dark:bg-teal-900/30 text-teal-800 dark:text-teal-200 border border-teal-200 dark:border-teal-700/70 px-2.5 py-1 rounded-lg text-xs font-bold inline-flex items-center gap-1.5 shadow-2xs">
                        ...
                        <div
                            ...
                            <div class="flex flex-wrap gap-1.5 overflow-hidden">
                                ...
                                <span
                                    class="min-w-0 max-w-full break-words bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-600 px-2 py-1 rounded-md text-[11px] font-medium text-slate-700 dark:text-slate-200 shadow-2xs">{{ $scheme }}</span>
                                @endforeach
                            </div>
                        </div>
                        @endif
