...
@php
    $isHindi = $locale === 'hi';
    $stepLabels = [
        1 => $isHindi ? 'बेसिक जानकारी' : 'Basic details',
        2 => $isHindi ? 'लक्षण चुनें' : 'Choose symptoms',
        3 => $isHindi ? 'रिव्यू और जांच' : 'Review & analyze',
    ];
@endphp
            ...
            <div class="relative grid gap-8 lg:grid-cols-[1.2fr_0.8fr] lg:items-center">
                <div class="max-w-3xl">
                    ...
                    <h1 class="mt-4 text-3xl font-extrabold tracking-tight text-slate-950 dark:text-white sm:text-5xl">
                        {{ $locale === 'hi' ? 'सही दिशा में जल्दी पहुंचें' : 'Get to the right direction faster' }}
                    </h1>
                    <p class="mt-4 max-w-2xl text-base leading-7 text-slate-600 dark:text-slate-300 sm:text-lg">
                        {{ $locale === 'hi'
                            ? 'तीन छोटे चरणों में उम्र, लिंग और लक्षण दर्ज करें। हम संभावित निदान, संबंधित मेडिकल विभाग और आगे पूछे जाने वाले लक्षण दिखाएंगे।'
                    ...
                    <div class="mt-6 flex flex-wrap gap-3">
                        <div class="inline-flex items-center gap-2 rounded-2xl border border-slate-200 bg-white/80 px-4 py-2 text-sm font-semibold text-slate-700 shadow-sm dark:border-slate-800 dark:bg-slate-950/70 dark:text-slate-200">
                            ...
                            {{ $locale === 'hi' ? 'कदम-दर-कदम फ्लो' : 'Step-by-step flow' }}
                        </div>
                        <div class="inline-flex items-center gap-2 rounded-2xl border border-slate-200 bg-white/80 px-4 py-2 text-sm font-semibold text-slate-700 shadow-sm dark:border-slate-800 dark:bg-slate-950/70 dark:text-slate-200">
                            <i data-lucide="stethoscope" class="h-4 w-4 text-teal-600 dark:text-teal-300"></i>
                            {{ $locale === 'hi' ? 'डिपार्टमेंट सुझाव' : 'Department guidance' }}
                        </div>
                        <div class="inline-flex items-center gap-2 rounded-2xl border border-slate-200 bg-white/80 px-4 py-2 text-sm font-semibold text-slate-700 shadow-sm dark:border-slate-800 dark:bg-slate-950/70 dark:text-slate-200">
                ...
                <div class="rounded-3xl border border-slate-200/80 bg-white/85 p-5 shadow-lg backdrop-blur dark:border-slate-800 dark:bg-slate-950/80 sm:p-6">
                    <div class="flex items-start gap-4">
                        ...
                        <div>
                            <p class="text-xs font-bold uppercase tracking-[0.24em] text-teal-700 dark:text-teal-300">{{ $locale === 'hi' ? 'क्या मिलेगा' : 'What you get' }}</p>
                            <h2 class="mt-1 text-xl font-extrabold text-slate-950 dark:text-white">{{ $locale === 'hi' ? 'साफ़ और उपयोगी परिणाम' : 'Clear and usable results' }}</h2>
                            <p class="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-300">
                                {{ $locale === 'hi'
                                    ? 'शीर्ष संभावित रोग, संबंधित विभाग, मैच करने वाले लक्षण और अगले पूछने वाले लक्षण एक ही जगह पर।'
            ...
            <section class="glass-card overflow-hidden rounded-[2rem] border border-slate-200/80 bg-white/95 shadow-xl dark:border-slate-800 dark:bg-slate-950/95">
                <div class="border-b border-slate-100 bg-gradient-to-r from-slate-50/90 via-white to-cyan-50/60 px-4 py-5 dark:border-slate-800 dark:from-slate-900 dark:via-slate-900 dark:to-slate-900 sm:px-8">
                    <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                        <div>
                            <p class="text-xs font-bold uppercase tracking-[0.24em] text-teal-700 dark:text-teal-300">{{ $locale === 'hi' ? 'स्टेप फ्लो' : 'Step flow' }}</p>
                            <h2 class="mt-1 text-2xl font-extrabold text-slate-950 dark:text-white">{{ $locale === 'hi' ? 'लक्षण जांच फॉर्म' : 'Symptom screening form' }}</h2>
                            <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">{{ $locale === 'hi' ? 'हर चरण पूरा करें और फिर आगे बढ़ें।' : 'Complete each step and move forward with confidence.' }}</p>
                        </div>
                        <div class="inline-flex items-center gap-3 rounded-2xl border border-slate-200 bg-white/90 px-4 py-3 shadow-sm dark:border-slate-800 dark:bg-slate-950/80">
                            <span class="text-xs font-bold uppercase tracking-[0.24em] text-slate-400">{{ $locale === 'hi' ? 'चरण' : 'Step' }}</span>
                    ...
                    <div class="mt-5 flex gap-2 overflow-x-auto pb-1 md:grid md:grid-cols-3 md:overflow-visible md:pb-0">
                        @foreach([1, 2, 3] as $stepNumber)
                            <div class="step-indicator min-w-[220px] md:min-w-0 flex items-center gap-3 rounded-2xl border px-4 py-3 text-sm font-semibold transition-all {{ $stepNumber === 1 ? 'border-teal-500 bg-teal-50 text-teal-800 dark:border-teal-500/40 dark:bg-teal-950/30 dark:text-teal-200' : 'border-slate-200 bg-white text-slate-500 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-400' }}" data-step-indicator="{{ $stepNumber }}">
                                <span class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-xs font-extrabold {{ $stepNumber === 1 ? 'bg-teal-600 text-white' : 'bg-slate-100 text-slate-500 dark:bg-slate-800 dark:text-slate-300' }}">{{ $stepNumber }}</span>
                ...
                <form id="symptom-test-form" class="space-y-6 px-4 py-6 sm:px-8 sm:py-8">
                        ...
                        <div class="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
                            ...
                            <div class="grid grid-cols-2 gap-2 sm:grid-cols-3 sm:gap-3 xl:min-w-[300px]">
                                <div class="rounded-2xl border border-slate-200/80 bg-white/90 px-3 py-3 text-center shadow-sm dark:border-slate-800 dark:bg-slate-950/80">
                                    <p class="text-[10px] font-bold uppercase tracking-[0.18em] text-slate-400">{{ $locale === 'hi' ? 'उम्र' : 'Age' }}</p>
                    ...
                    <div data-step-panel="1" class="space-y-6">
                        <div class="grid gap-5 md:grid-cols-2">
                            <div class="rounded-3xl border border-slate-200/80 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-950">
                                <label class="mb-2 block text-sm font-bold text-slate-700 dark:text-slate-200">{{ $locale === 'hi' ? 'उम्र' : 'Age' }}</label>
                                ...
                                <p class="mt-2 text-xs text-slate-500 dark:text-slate-400">{{ $locale === 'hi' ? 'सही उम्र से परिणाम बेहतर होते हैं।' : 'Accurate age helps improve the result.' }}</p>
                            </div>
                            <div class="rounded-3xl border border-slate-200/80 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-950">
                                ...
                                <select id="gender-select" name="gender" required class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3.5 text-slate-950 focus:border-teal-500 focus:ring-4 focus:ring-teal-500/10 dark:border-slate-700 dark:bg-slate-900 dark:text-white">
                                    ...
                                    <option value="unknown">{{ $locale === 'hi' ? 'न बताना चाहें' : 'Prefer not to say' }}</option>
                                </select>
                                <p class="mt-2 text-xs text-slate-500 dark:text-slate-400">{{ $locale === 'hi' ? 'कुछ स्थितियों में लिंग-आधारित संकेत उपयोगी होते हैं।' : 'Some conditions use gender-based signals to rank matches.' }}</p>
                            </div>
                        </div>
                    ...
                    <div data-step-panel="2" class="hidden space-y-6">
                        <div class="rounded-3xl border border-slate-200/80 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-950">
                            ...
                            <input id="symptom-filter" type="text" placeholder="{{ $locale === 'hi' ? 'जैसे: बुखार, खांसी, सिरदर्द...' : 'Try fever, cough, headache...' }}" class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3.5 text-slate-950 placeholder:text-slate-400 focus:border-teal-500 focus:ring-4 focus:ring-teal-500/10 dark:border-slate-700 dark:bg-slate-900 dark:text-white">
                        </div>
                        ...
                        <div class="rounded-3xl border border-slate-200/80 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-950">
                            <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                                <div>
                                    ...
                                    <p class="text-sm text-slate-500 dark:text-slate-400">{{ $locale === 'hi' ? 'कम से कम 1 लक्षण चुनें।' : 'Pick at least one symptom.' }}</p>
                                </div>
                                <p class="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">{{ $locale === 'hi' ? 'मोबाइल फ्रेंडली चयन' : 'Mobile-friendly selection' }}</p>
                            </div>
                        ...
                        <div class="rounded-3xl border border-slate-200/80 bg-gradient-to-r from-white to-teal-50/40 p-5 shadow-sm dark:border-slate-800 dark:from-slate-950 dark:to-teal-950/10">
                            <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                                <div>
                                    <h3 class="text-base font-extrabold text-slate-950 dark:text-white">{{ $locale === 'hi' ? 'चुने गए लक्षण' : 'Selected symptoms' }}</h3>
                                ...
                                <p id="selected-symptoms-count" class="inline-flex items-center rounded-full border border-teal-200 bg-white px-3 py-1 text-xs font-bold uppercase tracking-[0.18em] text-teal-700 shadow-sm dark:border-teal-900/40 dark:bg-slate-950 dark:text-teal-300">0 {{ $locale === 'hi' ? 'चुने गए' : 'selected' }}</p>
                            </div>
                    ...
                    <div data-step-panel="3" class="hidden space-y-6">
                        <div class="rounded-3xl border border-slate-200/80 bg-gradient-to-r from-slate-50 via-white to-cyan-50/60 p-5 shadow-sm dark:border-slate-800 dark:from-slate-900 dark:via-slate-900 dark:to-slate-900">
                            <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                                <div class="min-w-0">
                                    <h3 class="text-lg font-extrabold text-slate-950 dark:text-white">{{ $locale === 'hi' ? 'जांच से पहले रिव्यू' : 'Review before analysis' }}</h3>
                                    <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">{{ $locale === 'hi' ? 'यहां पुष्टि करें कि आपने सही जानकारी चुनी है।' : 'Confirm your details here before running the analysis.' }}</p>
                                </div>
                            </div>
                            <dl class="mt-4 grid gap-4 sm:grid-cols-2">
                                <div class="rounded-2xl border border-slate-200/80 bg-white px-4 py-4 shadow-sm dark:border-slate-800 dark:bg-slate-950">
                                    <dt class="text-xs font-bold uppercase tracking-[0.18em] text-slate-400">{{ $locale === 'hi' ? 'उम्र' : 'Age' }}</dt>
                            ...
                            <div class="mt-4">
                                <p class="text-xs font-bold uppercase tracking-[0.18em] text-slate-400">{{ $locale === 'hi' ? 'चुने गए लक्षण' : 'Selected symptoms' }}</p>
                                <div id="review-symptoms" class="mt-2 flex flex-wrap gap-2"></div>
                            </div>
                        </div>

                        <div class="rounded-3xl border border-slate-200/80 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-950">
                            <label class="mb-2 block text-sm font-bold text-slate-700 dark:text-slate-200">{{ $locale === 'hi' ? 'अतिरिक्त जानकारी' : 'Additional notes' }}</label>
                            <textarea id="symptom-text" name="symptom_text" rows="4" placeholder="{{ $locale === 'hi' ? 'जैसे: कितने दिन से है, क्या बढ़ रहा है, कोई नई परेशानी?' : 'For example: how long, getting worse, any extra detail?' }}" class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3.5 text-slate-950 placeholder:text-slate-400 focus:border-teal-500 focus:ring-4 focus:ring-teal-500/10 dark:border-slate-700 dark:bg-slate-900 dark:text-white"></textarea>
                        </div>
                        ...
                        <div class="hidden flex-col-reverse gap-3 sm:flex sm:flex-row sm:items-center sm:justify-between">
                            ...
                            <button type="submit" class="inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-teal-600 to-cyan-600 px-5 py-3.5 text-sm font-extrabold text-white shadow-lg shadow-teal-600/20 transition hover:from-teal-700 hover:to-cyan-700 sm:w-auto">
                                ...
                                {{ $locale === 'hi' ? 'जांच शुरू करें' : 'Run analysis' }}
                            </button>
                        </div>
                    </div>
                </form>
            ...
            <aside id="results-section" class="order-last space-y-4 xl:sticky xl:top-24 xl:order-none xl:self-start">
                <div class="rounded-[2rem] border border-slate-200/80 bg-white/95 p-6 shadow-xl dark:border-slate-800 dark:bg-slate-950/95">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <h2 class="mt-1 text-lg font-extrabold text-slate-950 dark:text-white">{{ $locale === 'hi' ? 'निदान और सुझाव' : 'Diagnosis & guidance' }}</h2>
                        </div>
                    </div>
                    ...
                    <div id="results-panel" class="mt-6 space-y-4">
                        <div class="rounded-3xl border border-dashed border-slate-300 bg-gradient-to-b from-slate-50 to-cyan-50/60 p-5 dark:border-slate-700 dark:from-slate-900 dark:to-slate-900">
                            ...
                            <p class="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-300">
                                {{ $locale === 'hi'
                                    ? 'उम्र, लिंग और कम से कम 1 लक्षण चुनें। फिर हम संभावित निदान, संबंधित विभाग और अगले पूछे जाने वाले लक्षण दिखाएंगे।'
                ...
                <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-1">
                    <div class="rounded-[1.5rem] border border-slate-200/80 bg-white/95 p-5 shadow-sm dark:border-slate-800 dark:bg-slate-950/95">
                        <h3 class="font-extrabold text-slate-950 dark:text-white">{{ $locale === 'hi' ? 'कैसे मदद करता है' : 'How it helps' }}</h3>
                        <p class="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-300">{{ $locale === 'hi' ? 'जितने बेहतर लक्षण चुनेंगे, उतनी बेहतर संभावित दिशा और विभाग सुझाव मिलेंगे।' : 'The better your symptom selection, the more useful the likely direction and department guidance becomes.' }}</p>
                    </div>
                    <div class="rounded-[1.5rem] border border-slate-200/80 bg-white/95 p-5 shadow-sm dark:border-slate-800 dark:bg-slate-950/95">
                        <h3 class="font-extrabold text-slate-950 dark:text-white">{{ $locale === 'hi' ? 'महत्वपूर्ण' : 'Important' }}</h3>
                        <p class="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-300">{{ $locale === 'hi' ? 'यह मेडिकल स्क्रीनिंग टूल है, अंतिम निदान नहीं। गंभीर लक्षण हों तो तुरंत डॉक्टर से संपर्क करें।' : 'This is a medical screening tool, not a final diagnosis. If symptoms are serious, contact a doctor immediately.' }}</p>
                    </div>
                </div>
            </aside>
        </div>
...
<div class="sm:hidden fixed inset-x-0 bottom-0 z-40 border-t border-slate-200/80 bg-white/95 px-4 py-3 shadow-[0_-12px_30px_rgba(15,23,42,0.08)] backdrop-blur dark:border-slate-800 dark:bg-slate-950/95">
    <div class="flex items-center gap-3">
        ...
        <button type="button" id="mobile-next-button" class="flex-1 items-center justify-center gap-2 rounded-2xl bg-slate-950 px-4 py-3 text-sm font-bold text-white dark:bg-white dark:text-slate-950">
            <span id="mobile-next-label">{{ $locale === 'hi' ? 'अगला चरण' : 'Next step' }}</span>
            ...
            <div class="rounded-3xl border border-teal-200 bg-gradient-to-br from-teal-50 to-cyan-50 p-5 dark:border-teal-900/40 dark:bg-teal-500/10">
                ...
                <div class="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-3">
                    <div class="rounded-2xl bg-white/80 px-4 py-3 dark:bg-slate-950/70">
                        ...
                        <p class="mt-1 text-sm font-bold text-slate-950 dark:text-white">${recommendedDepartment ? (recommendedDepartment.name?.[isHindi ? 'hi' : 'en'] || recommendedDepartment.name?.en || '') : translate('General evaluation', 'सामान्य जांच')}</p>
                    </div>
                    <div class="rounded-2xl bg-white/80 px-4 py-3 dark:bg-slate-950/70">
