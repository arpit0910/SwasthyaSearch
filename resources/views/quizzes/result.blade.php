    ...
    <section class="rounded-[2rem] border border-violet-100/80 bg-gradient-to-br from-white via-violet-50/40 to-cyan-50/50 p-6 shadow-sm dark:border-slate-800 dark:from-slate-900/90 dark:via-slate-900/90 dark:to-slate-950 sm:p-8">
        <div class="grid gap-6 lg:grid-cols-[1fr_320px] lg:items-start">
            <div>
                ...
                <h1 class="mt-3 text-3xl font-extrabold text-slate-950 dark:text-white">{{ $isHindi ? 'एक शांत नज़र से देखें' : 'Read this with a gentle lens' }}</h1>
                <p class="mt-3 text-sm sm:text-base leading-7 text-slate-600 dark:text-slate-300">
                    {{ $isHindi ? 'यह self-reflection के लिए है, फैसला सुनाने के लिए नहीं। जो useful लगे, वह लें।' : 'This is for self-reflection, not for judging you. Take what feels useful and leave what does not.' }}
                </p>
            </div>

            <div class="rounded-[1.6rem] border border-white/80 bg-white/90 p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900/90">
                <div class="text-sm text-slate-500 dark:text-slate-400">{{ $isHindi ? 'स्कोर' : 'Score' }}</div>
        ...
        <div class="mt-6 grid gap-5 md:grid-cols-3">
            @foreach([
                [
                    'title' => $isHindi ? 'एक मिनट शांत साँस' : 'One minute breathing',
                    'description' => $isHindi ? 'अगर इस परिणाम के बाद मन तेज़ लग रहा है, पहले ब्रीदिंग खोलें।' : 'If reading this made your mind race, open breathing first.',
                ...
                [
                    ...
                    'description' => $isHindi ? 'ambient sound और gentle voice के साथ थोड़ी राहत लें।' : 'Settle with ambient sound and a softer voice-guided pause.',
                ...
                [
                    ...
                    'description' => $isHindi ? 'अगर आप थोड़ी clarity चाहते हैं, mood check-in करें।' : 'If you want a little more clarity, try the mood check-in next.',
                ...
                <a href="{{ $card['url'] }}" class="rounded-[1.5rem] border border-slate-200/80 bg-white/90 p-5 shadow-sm transition hover:-translate-y-0.5 dark:border-slate-800 dark:bg-slate-900/90">
                    ...
                    <span class="mt-4 inline-flex text-sm font-bold text-violet-700 dark:text-violet-300">{{ $isHindi ? 'खोलें' : 'Open' }}</span>
                </a>
            @endforeach
        </div>
        ...
        <div class="mt-6 flex flex-wrap gap-3">
            <a href="{{ route('quizzes.index') }}" class="rounded-2xl border border-slate-200 px-5 py-3 text-sm font-bold text-slate-900 dark:border-slate-700 dark:text-slate-100">{{ $isHindi ? 'और क्विज़ देखें' : 'Explore more quizzes' }}</a>
            <a href="{{ route('support.crisis') }}" class="rounded-2xl bg-slate-950 px-5 py-3 text-sm font-bold text-white dark:bg-white dark:text-slate-950">{{ $isHindi ? 'अगर unsafe लगे तो support लें' : 'Get support if you feel unsafe' }}</a>
        </div>
    </section>
</main>
@endsection
